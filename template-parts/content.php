	...
	<div class="entry-content-wrap">
			...
			if ( is_single() && ! is_related_posts() ) { ?>
				<?php if ( has_post_thumbnail() ) { ?>
					<figure class="entry-media entry-img bb-vw-container1">
						<?php the_post_thumbnail( 'post-thumbnail', [ 'sizes' => '(max-width:768px) 768px, (max-width:1024px) 1024px, (max-width:1920px) 1920px, 1024px' ] ); ?>
					</figure>
				<?php } ?>
			<?php } ?>
				...
				<?php if ( has_post_thumbnail() ) { ?>
					<figure class="entry-media entry-img bb-vw-container1">
						<?php if ( ! empty( $featured_img_style ) && $featured_img_style == "full-fi" ) {
							the_post_thumbnail( 'post-thumbnail', [ 'sizes' => '(max-width:768px) 768px, (max-width:1024px) 1024px, (max-width:1920px) 1920px, 1024px' ] );
						} else {
		...
		<?php if ( is_singular() && ! is_related_posts() ) { ?>
			...
			<?php
				the_content( sprintf (
					wp_kses(
					__( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'bbtc' ), [
						'span' => [
							'class' => [],
						],
					]
	...
	<div class="post-meta-wrapper">
		<?php if  ( has_category() ) : ?>
			<div class="cat-links">
				...
				<?php _e( 'Categories: ', 'bbtc' ); ?>
				<span><?php the_category( __( ', ', 'bbtc' ) ); ?></span>
			</div>
		<?php endif; ?>
		...
		<?php if  ( has_tag() ) : ?>
			<div class="tag-links">
				...
				<?php _e( 'Tagged: ', 'bbtc' ); ?>
				<?php the_tags( '<span>', __( ', ', 'bbtc' ),'</span>' ); ?>
			</div>
		<?php endif; ?>
	</div>
<?php } ?>
